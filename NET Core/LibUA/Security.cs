...
namespace LibUA
{
    public static class UASecurity
    {
        ...
        public static int CalculatePaddingSize(X509Certificate2 cert, SecurityPolicy policy, int position, int sigSize)
        {
            ...
            int plainBlockSize = GetPlainBlockSize(cert, useOaep);
        ...
        public static string ExportPEM(X509Certificate cert)
        {
            ...
            sb.AppendLine(Convert.ToBase64String(cert.Export(X509ContentType.Cert),
                ...
                using (var crypt = aes.CreateDecryptor(aes.Key, aes.IV))
                {
                    using (var ms = new MemoryStream(data.Array, data.Offset, data.Count))
                    {
                        ...
                        using (var msRead = new MemoryStream(plain))
                        {
                            ...
                            int length = lengthBytes[0] | (lengthBytes[1] << 8) | (lengthBytes[2] << 16) |
                                         (lengthBytes[3] << 24);
        ...
        public static int RijndaelEncryptInplace(ArraySegment<byte> data, byte[] key, byte[] iv)
        {
            ...
            })
            {
                using (var crypt = rijn.CreateEncryptor(rijn.Key, rijn.IV))
                {
                    ...
                    {
                        throw new Exception(string.Format("Input data is not a multiple of block size, {0}/{1}",
                            data.Count, crypt.InputBlockSize));
                    }
        ...
        {
            bool useOaep = policy == SecurityPolicy.Basic256;
            return GetSignatureLength(cert, useOaep);
        }

        public static byte[] RsaPkcs15Sha1_Sign(ArraySegment<byte> data, RSA privProvider)
        {
            var hash = new SHA1Managed();
            var digest = hash.ComputeHash(data.Array, data.Offset, data.Count);

            byte[] signature = privProvider.SignHash(digest, HashAlgorithmName.SHA1, RSASignaturePadding.Pkcs1);
            return signature;
        }

        public static bool RsaPkcs15Sha1_VerifySigned(ArraySegment<byte> data, byte[] signature, X509Certificate2 cert)
        {
            var rsa = cert.PublicKey.Key as RSA;
            ...
            bool match = rsa.VerifyHash(digest, signature, HashAlgorithmName.SHA1, RSASignaturePadding.Pkcs1);
            return match;
        }
        ...
        public static byte[] RsaPkcs15Sha1_Decrypt(ArraySegment<byte> data, X509Certificate2 cert,
            RSA rsaPrivate, SecurityPolicy policy)
        {
            bool useOaep = policy == SecurityPolicy.Basic256;

            int cipherBlockSize = GetCipherTextBlockSize(cert, useOaep);
            int plainSize = data.Count / cipherBlockSize;
            int blockSize = GetPlainBlockSize(cert, useOaep);

            plainSize *= blockSize;

            var buffer = new byte[plainSize];
            int inputBlockSize = rsaPrivate.KeySize / 8;
            int outputBlockSize = GetPlainBlockSize(cert, useOaep);
            ...
            for (int i = data.Offset; i < data.Offset + data.Count; i += inputBlockSize)
            {
                ...
                var plain = rsaPrivate.Decrypt(block, useOaep?RSAEncryptionPadding.OaepSHA1 : RSAEncryptionPadding.Pkcs1);
        ...
        public static byte[] SHA1Calculate(byte[] data)
        {
        ...
        public static byte[] SymmetricSign(byte[] key, ArraySegment<byte> data)
        {
        ...
        public static byte[] SHA1Calculate(ArraySegment<byte> data)
        {
            using (SHA1Managed sha1 = new SHA1Managed())
            {
                return sha1.ComputeHash(data.Array, data.Offset, data.Count);
            }
        }

        public static bool SHA1Verify(byte[] data, byte[] hash)
        {
            var calc = SHA1Calculate(data);
        ...
        public static byte[] PSHA1(byte[] secret, byte[] seed, int length)
        {
            ...
            var keySeed = new byte[Sha1Size + seed.Length];
            ...
            while (pos < length)
            {
                ...
                int writeLen = Math.Min(Sha1Size, length - pos);
        ...
        public static StatusCode UnsecureSymmetric(MemoryBuffer recvBuf, uint tokenID, uint? prevTokenID,
            int messageEncodedBlockStart, SLChannel.Keyset localKeyset, SLChannel.Keyset[] remoteKeysets,
            SecurityPolicy policy, MessageSecurityMode securityMode, out int decrSize)
        {
            decrSize = -1;
            ...
            if (securityMode == MessageSecurityMode.SignAndEncrypt)
            {
                try
                {
                    decrSize = UASecurity.RijndaelDecryptInplace(
                                   new ArraySegment<byte>(recvBuf.Buffer, messageEncodedBlockStart,
                                       (int) messageSize - messageEncodedBlockStart),
                                   remoteKeysets[keysetIdx].SymEncKey, remoteKeysets[keysetIdx].SymIV) +
                               messageEncodedBlockStart;
            ...
            if (securityMode >= MessageSecurityMode.Sign)
            {
                try
                {
                    ...
                    var sigExpect = UASecurity.SymmetricSign(remoteKeysets[keysetIdx].SymSignKey, sigData);
        ...
        public static StatusCode SecureSymmetric(MemoryBuffer respBuf, int messageEncodedBlockStart,
            SLChannel.Keyset localKeyset, SLChannel.Keyset remoteKeyset, SecurityPolicy policy,
            MessageSecurityMode securityMode)
        {
            ...
            if (securityMode >= MessageSecurityMode.SignAndEncrypt)
            {
                ...
                int padSize = CalculateSymmetricPaddingSize(localKeyset.SymEncKey.Length,
                    sigSize + respBuf.Position - messageEncodedBlockStart);
            ...
            if (securityMode >= MessageSecurityMode.SignAndEncrypt)
            {
                msgSize = messageEncodedBlockStart + CalculateSymmetricEncryptedSize(localKeyset.SymEncKey.Length,
                    msgSize - messageEncodedBlockStart);
            }
